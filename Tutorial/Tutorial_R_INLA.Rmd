...
Let assume for example that we are interested in extracting and plotting the marginal posterior distribuion of $b_1$:
...
It is possible to extract more information from the posterior distribution of $b_1$:
```{r}
# Quantile function
inla.qmarginal(0.05,beta1_post) 
# Distribution function
inla.pmarginal(0.1,beta1_post)
# Density function
...
We obtain that an increase of 1 unit in the `x1` covariate is associated with an increase of around 12\% in the risk of suicide.
...
length(output2$marginals.random)
...
For the spatially struttured random effect we consider the following prior distribution $\mathbf u ∼ ICAR(\mathbf W, 1/\tau_u)$ where $\mathbf W$ is the neighbourhood matrix.

For both the precision hyperparameters $\tau_v$ (`prec.unstruct`) and $\tau_v$ (`prec.spatial`) we specify a logGamma(1,0.1) prior distribution.
